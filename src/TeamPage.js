...
const MemberRow = props => {
  ...
  const bio = props.bio.content[0].content[0].value

  return (
    <div className="icon-row">
      <div className="member-container">
        <div className="member-circle">
        <Image imageURL={props.pic.sys.id} />
        </div>
      </div>
      <div className="bio-container">
        <h1 style={{fontSize: '40px !important'}}>{props.name}</h1>
        <p className="medium-spacing">
          {isOpen ? bio : bio.substring(0, 600) + ' '}

          {bio.length > 500 && !isOpen && (
            <span
              className="read-more"
          ...
          {/* {documentToReactComponents(props.bio)} */}
        </p>
...
const TeamPage = () => {
  ...
  const getMembers = () => {
    axios.get('https://cdn.contentful.com/spaces/fxjn6hn7ptqk/environments/master/entries?access_token=7Cp2qRcbgPNokFoPZjpxbZKyYewJdeCypE94i6Lm71Q&content_type=member&order=sys.createdAt').then((res) => {
      console.log(res)
